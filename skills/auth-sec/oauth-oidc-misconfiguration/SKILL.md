...
## 4. ACCOUNT TAKEOVER VIA REFERER

Some OAuth/OIDC callbacks or connect endpoints return tokens in the response and are vulnerable to Referer-based token leakage:

```
GET /auth/connect HTTP/1.1
Host: target.com
Referer: https://attacker.com/
```

If the response includes a valid `access_token` or `code` and the endpoint does not validate the `Referer` / `Origin` header, the token can be captured by an attacker-controlled page via:

- `<img>` or `<script>` tag embedding on attacker's page → Referer leaks full URL
- Open redirect on the target domain → chain with Referer to exfiltrate tokens
- CORS misconfiguration allowing credentialed cross-origin reads of the connect endpoint
